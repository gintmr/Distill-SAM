...
from eval_tools import calculate_pa_iou, inference_image, init_model, overlay_mask_on_image
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="/data2/wuxinrui/RA-L/MobileSAM/eval_results", help="path to save the output")
    parser.add_argument("--image_size", type=int, default=1024, help="image size")
    parser.add_argument("--visualize_mask_path", default="/data2/wuxinrui/RA-L/MobileSAM/eval_results")
    ...
    for img_file in tqdm(img_files):
        ...
        for bbox_pred_mask in bbox_pred_masks:
            overlay = cv2.imread(img_path)
            overlay = overlay_mask_on_image(bbox_pred_mask, output_path_bbox, image_array=overlay)
        cv2.imwrite(output_path_bbox, overlay)
        for point_pred_mask in point_pred_masks:
            overlay = cv2.imread(img_path)
            overlay = overlay_mask_on_image(point_pred_mask, output_path_point, image_array=overlay)
        cv2.imwrite(output_path_point, overlay)
