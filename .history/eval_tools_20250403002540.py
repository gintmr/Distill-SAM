...
def overlay_point_on_image(center_points, output_path, image_path=None, image_array=None):
    """
    将点叠加到原图上并保存。
...
def overlay_mask_on_image(mask, output_path, image_path=None, image_array=None, mask_color=(178, 102, 255), alpha=0.5):
    ...
    if image_array is not None:
        ...
        print("Error: Both image_path and image_array are None")
        return

    # 转换图像格式
    if isinstance(image, torch.Tensor):
        image = image.cpu().numpy()
    if len(image.shape) == 3 and image.shape[0] in [1, 3, 4]:  # CHW转HWC
        image = image.transpose(1, 2, 0)
    elif len(image.shape) == 2:  # 灰度图转RGB
    ...
    if isinstance(mask, list) or len(mask.shape) == 3:
        masks = mask if isinstance(mask, list) else [mask[i] for i in range(mask.shape[0])]
        colors = np.random.randint(0, 256, size=(len(masks), 3), dtype=np.uint8)
        
        for i, mask_s in enumerate(masks):
            # 确保mask是二维的
            if len(mask_s.shape) == 3 and mask_s.shape[2] == 1:
                mask_s = mask_s[:, :, 0]
            
            # 创建彩色掩码
            color_mask = np.zeros_like(image, dtype=np.uint8)
            color_mask[mask_s > 0] = colors[i] if mask_color is None else mask_color
            
            # 叠加到最终掩码
            final_mask = cv2.add(final_mask, color_mask)
    ...
    # 将掩码叠加到原图
    overlay = cv2.addWeighted(overlay, 1 - alpha, final_mask, alpha, 0)
...
#             mask_image[:mask_3channel.shape[0],:mask_3channel.shape[1],:] = mask_3channel
#             mask_image[mask_image[:,:,0] > 0] = mask_color
