...
class MobileSAMFintuner(pl.LightningModule):
    ...
    def forward(self, imgs, bboxes, labels, center_points, point_labels, target_labels, original_input_size, resized_input_size, coco_image_names):
        ...
        try:
            ...
            for feature, bbox, label, center_point, point_label, target_label, original_input_size_item, resized_input_size_item in \
                ...
                loss_focal += sigmoid_focal_loss(masks, label.float(), num_masks, alpha=0.6)
            ...
            return {
                'loss': 15. * loss_focal + 5*loss_dice + 3*loss_iou,  # SAM default loss
    ...
    def configure_optimizers(self):
        ...
        scheduler = CosineAnnealingLR(
            opt,
            T_max=self.trainer.estimated_stepping_batches,  # 总步数
            eta_min=1e-6  # 最小学习率
        )
