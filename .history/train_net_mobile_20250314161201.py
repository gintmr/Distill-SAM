...
from mobile_sam.utils import ResizeLongestSide
...
class Coco2MaskDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for annotation in coco_image.annotations[:self.length]:
            ...
            points, num_points = random_croods_in_mask(mask = mask, num_points=self.num_points)
...
def main():
    ...
    parser.add_argument("--train_data", default="/data2/wuxinrui/Projects/ICCV/MIMC_FINAL/seen/train_list", type=str, required=True, help="path to the data root")
    parser.add_argument("--val_data", default="/data2/wuxinrui/Projects/ICCV/MIMC_FINAL/seen/val_list", type=str, required=True, help="path to the data root")
    ...
    # load the dataset
    train_dataset = Coco2MaskDataset(data_root=, annotation_path=, image_size=args.image_size,
                                     length=args.length,num_points=args.num_points,use_centerpoint=args.use_centerpoint)
    val_dataset = Coco2MaskDataset(data_root=, annotation_path=, image_size=args.image_size,
                                   length=args.length, num_points=args.num_points,use_centerpoint=args.use_centerpoint)
    # create the model
