    ...
    def __getitem__(self, index):
        try:
            ...
            for annotation in annotations[:self.length]:
                ...
                points = [self.transform.apply_coords_torch(point, original_input_size) for point in points]
                for point in points:
                    point = self.transform.apply_coords_torch(point, original_input_size)
                bbox = self.transform.apply_boxes_torch(bbox, original_input_size)
                ## bbox需要resize,同理center_points需要resize, points也需要resize
...
def main():
    ...
    parser.add_argument("--use_bbox", defualt=True, help="generate multi masks")
    ...
    parser.add_argument("--log_dir", default="./metrics_logs")
