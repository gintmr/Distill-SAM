...
def feature_distillation_loss(T_features, S_features, T_layers_features, S_layers_features, reduction='mean', alpha=0.5, beta=0.01):
    """
    计算教师模型和学生模型特征的蒸馏损失。
    ...
    返回:
        torch.Tensor: 蒸馏损失。
    """
    temperature = max(3.0 - 2*(current_step/total_steps), 1.0)
    ...
    def forward(self, imgs, bboxes, labels, center_points, point_labels, target_labels, original_input_size, resized_input_size, coco_image_names):
        ...
        try:
            with autocast():
                ...
                distill_loss = feature_distillation_loss(T_features, S_features, T_layers_features, S_layers_features)
