...
def overlay_mask_on_image(mask, output_path, image_path=None, image_array=None, mask_color=(178, 102, 255), alpha=0.5):
    ...
    if isinstance(mask, list) or len(mask.shape) == 3:
        ...
        for mask_s, mask_color in zip(mask, colors):
            ...
            pad_height = max(0, mask_image.shape[0] - mask_3channel.shape[0])
            pad_width = max(0, mask_image.shape[1] - mask_3channel.shape[1])
            mask_3channel_padded = np.pad(mask_3channel, 
                                           ((0, pad_height), (0, pad_width), (0, 0)), 
                                           mode='constant', constant_values=0)
            mask_image[mask_3channel_padded > 0] = mask_color
    else:
