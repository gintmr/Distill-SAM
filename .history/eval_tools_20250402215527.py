...
def random_croods_in_mask(mask, num_croods=1):
    ...
    selected_croods = croods_to_chose[np.random.choice(len(croods_to_chose).permute(1, 0), num_croods, replace=False)]
...
def overlay_mask_on_image(mask, output_path, image_path=None, image_array=None, mask_color=(178, 102, 255), alpha=0.5):
    ...
    if isinstance(mask, list) or len(mask.shape) == 3:
        ...
        for mask_s, mask_color in zip(mask, colors):
            ...
            mask_3channel = mask_s[:, :, np.newaxis].repeat(3, axis=2)
