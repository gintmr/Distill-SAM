...
def random_croods_in_mask(mask, num_croods=1):
    ...
    '''
    croods_coords = np.argwhere(mask > 0)
    
    if len(croods_coords) < num_croods:
        return croods_coords, len(croods_coords)
    selected_croods = croods_coords[np.random.choice(len(croods_coords), num_croods, replace=False)]
    return selected_croods