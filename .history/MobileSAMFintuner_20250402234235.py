...
class MobileSAMFintuner(pl.LightningModule):
    ...
    def forward(self, imgs, bboxes, labels, center_points, point_labels, target_labels, original_input_size, resized_input_size, coco_image_names):
        ...
        for feature, bbox, label, center_point, point_label, target_label, original_input_size_item, resized_input_size_item, coco_image_name, img in \
            ...
            #G original_input_size_item => H,W
            # resized_center_point = self.transform.apply_coords_torch(center_point, original_input_size_item)
            # resized_bbox = self.transform.apply_boxes_torch(bbox, original_input_size_item)
            if self.use_bbox:
                bbox_input=resized_bbox
            else:
                bbox_input=None


            sparse_embeddings, dense_embeddings = self.model.prompt_encoder(
                points=(resized_center_point, point_label),  #
                boxes=bbox_input,
                masks=None,
            )
            torch.cuda.empty_cache()
            ...
            output_path_pred = os.path.join(training_visual_path, coco_image_name.replace(ext_name, "_pred.png"))
            output_path_GT = os.path.join(training_visual_path, coco_image_name.replace(ext_name, "_GT.png"))
            output_path_point = os.path.join(training_visual_path, coco_image_name.replace(ext_name, "_point.png"))
