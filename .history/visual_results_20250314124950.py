...
mask_generator, mask_predictor = init_model(model_type="vit_t", sam_checkpoint="./weights/mobile_sam.pt", device="cuda", generator=True, predictor=True, point_prompt=True)
# 遍历文件夹中的所有图片
for filename in os.listdir(input_folder)[:10]:
    if filename.endswith(('.jpg', '.jpeg', '.png')):
        ...
        results_dict = inference_image(image_path=image_path, mask_generator=mask_generator,mask_predictor=mask_predictor, device="cuda")
        generator_results = results_dict['generator_results']
        image = results_dict['image_array']
        

        # 将掩码叠加到原始图像上并保存
        overlay_masks_on_image(image, generator_results, output_path)
