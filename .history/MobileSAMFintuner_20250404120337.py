...
import cv2
from eval_tools import calculate_pa_iou, overlay_mask_on_image,overlay_point_on_image
        ...
        try:
            ...
            for feature, bbox, label, center_point, point_label, target_label, original_input_size_item, resized_input_size_item, coco_image_name, img in \
                ...
                if not os.path.exists(training_visual_path):
                    os.makedirs(training_visual_path)
                #G 1% to save images
                if random.random() < 0.01:
                    output_path_pred = os.path.join(training_visual_path, f"{step}_{coco_image_name.replace(ext_name, '_pred.png')}")
                    output_path_GT = os.path.join(training_visual_path, f"{step}_{coco_image_name.replace(ext_name, '_GT.png')}")
                    output_path_point = os.path.join(training_visual_path, f"{step}_{coco_image_name.replace(ext_name, '_point.png')}")

                    overlay_mask_on_image(pred_masks, output_path_pred, image_array=img)
                    overlay_mask_on_image(iou_label, output_path_GT, image_array=img)
                    overlay_point_on_image(center_point, output_path_point, image_array=img)

                    #G check to avoid too many images in the folder
                    image_files = [f for f in os.listdir(training_visual_path) if f.endswith(('_pred.png', '_GT.png', '_point.png'))]
                    if len(image_files) > 30:
                        #G randomly select three file prefixes
                        random_prefixes = random.sample(set(f.split('_GT.png')[0] for f in image_files), 3)
                        for prefix in random_prefixes:
                            for suffix in ('_pred.png', '_GT.png', '_point.png'):
            ...
            penalty_coefficient = 0.5 * (1 + av_BS_IoU)

            return {
                'loss': (20 * loss_focal + loss_dice + loss_iou) / penalty_coefficient,  # SAM default loss
